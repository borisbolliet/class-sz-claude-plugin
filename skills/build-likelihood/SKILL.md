---
description: Scaffold a cobaya likelihood for a tSZ Cl^yy power-spectrum bandpower dataset using classy_szlite (pure-JAX, ede-v2 default). Produces a Python likelihood + foreground theory + JAX theory module plus a working cobaya YAML ready to MCMC. The theory uses classy_szlite.cl_yy_factory for ~5 ms/eval (~3× faster than the full pipeline).
disable-model-invocation: true
argument-hint: "<likelihood-name> [--cosmo-model=ede-v2|lcdm] [--data-dir DIR]"
...
Scaffold a **classy_szlite**-backed Gaussian likelihood + foreground theory + JAX theory module and a working cobaya YAML for a **tSZ Cl^yy power-spectrum bandpower dataset** (data is binned bandpowers + N×N covariance — not y-map pixels). Output: a Python file and a YAML, both pointing at a local workdir.

Arguments in `$ARGUMENTS`:
- `$0` — likelihood class name root, CamelCase (e.g. `Clyy`). The skill writes three classes: `<Root>Likelihood`, `<Root>Theory`, `<Root>ForegroundTheory`.
- `--cosmo-model=ede-v2` (default) or `--cosmo-model=lcdm` — which CosmoPower emulator suite to use.
- `--data-dir DIR` — directory with the bandpower / cov / multipoles / foreground files (otherwise ask the user).

## Why classy_szlite

The scaffold uses [classy_szlite](https://github.com/CLASS-SZ/classy_szlite) — a pure-JAX subset of classy_szfast with only `jax + numpy + mcfit` deps. ede-v2 is the default emulator (supports both ΛCDM-equivalent at `fEDE=0.001` and full EDE parameter space); LCDM emulator available via `--cosmo-model=lcdm`.

For MCMC where cosmology is fixed and only profile / nuisance parameters are sampled, the scaffold uses `classy_szlite.cl_yy_factory` to precompute CosmoGrids + HaloGrids once at `initialize()` time — per-step cost is then just `cl_yy_1h_2h` integration, ~5 ms / eval.

EDE-specific parameters (`fEDE`, `log10z_c`, `thetai_scf`, `r`, `m_ncdm`, `N_ur`) are not surfaced on the cobaya Theory class — classy_szlite fills them silently with their LCDM-equivalent defaults. To override, the user can edit the Python module directly.
...
**Always run cobaya-run from the workdir** so the scaffolded module is on `sys.path`.

## Steps

1. **Read the data.** `np.loadtxt` the bandpower file (3-col: ell, D_ell·1e12, σ); confirm shape matches the cov file (N×N) and the multipoles file (N rows). Mismatched ell centers between data and multipoles is a silent bug — verify.
...
3. **Write the Python module** to `<workdir>/<likelihood-name>.py`. The reference shape (mirrors the validated `~/Desktop/class-sz-plugin-tests/clyy_v2.py`):

```python
"""<Root> — Cl^yy bandpower likelihood + JAX theory via classy_szlite."""
...
class <Root>Theory(Theory):
    """classy_szlite-backed Cl_sz provider with fixed-cosmology fast path.

    Uses cl_yy_factory: precompute CosmoGrids + HaloGrids once at init
    (cosmology is fixed for this Theory); per-step calls only the
    cl_yy_1h_2h integration. ~5 ms / eval after warmup.

    Cosmology surface: only the standard 6 LCDM params; EDE-specific
    parameters (fEDE, log10z_c, thetai_scf, r, m_ncdm, N_ur) are not
    surfaced — classy_szlite fills them with LCDM-equivalent defaults
    when cosmo_model='ede-v2' (the default).
    """
    cosmo_model:     str   = "ede-v2"          # default; or 'lcdm'
...
4. **Write the cobaya YAML** to `<workdir>/<likelihood-name>.yaml`. ede-v2 emulators by default; only the standard 6 cosmology params exposed:
...
theory:
  ...
  <module>.<Root>Theory:
    cosmo_model: ede-v2                  # default; switch to 'lcdm' for the lcdm emulator suite
    delta_crit: 500.0
    n_z: 100
    n_m: 200
...
6. **Report back**:
   ...
   - `--test` result + `Average evaluation time` for `<Root>Theory` (expect ~5-15 ms warm)
   - Suggested next: full chain (`cobaya-run` or `mpirun -np 4 cobaya-run`)

## Prerequisites

The user's venv must have `classy_szlite` installed. Install from the CLASS-SZ org repo:
...
Verify: `~/pyvenvs/py312-class_sz/bin/python -c "import classy_szlite; print(classy_szlite.__version__)"`

## Pitfalls

- Always run cobaya-run from `<workdir>` so the module is importable. Don't run from `~/GitHub` (PEP 420 cwd collision with the cobaya / classy_szlite repo subfolders shadows the editable installs).
- The `multipoles` file must use the SAME ell centers as the bandpower data file — verify with `np.loadtxt`.
- For EDE-mode work (sampling `fEDE` etc.), edit `<Root>Theory.initialize` to expose those as class attrs and pass them to `csl.CosmoParams(...)`. The default scaffold keeps EDE silent at the LCDM-equivalent point.
- `cl_yy_factory` precomputes CosmoGrids + HaloGrids ONCE — if you want to sample cosmology too, replace the factory call with the full `csl.cl_yy(...)` pipeline (slower, ~15 ms/eval).
